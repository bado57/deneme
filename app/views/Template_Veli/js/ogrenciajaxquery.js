...
$.ajaxSetup({
    type: "post",
    url: SITE_URL + "VeliMobilOgrenciAjax",
    //timeout:3000,
    dataType: "json",
    error: function (a, b) {
        if (b == "timeout")
...
ons.ready(function () {
    ...
    $(document).on('pageinit', '#islemlistepage', function () {
        ...
        var ogrid = page.options.ogrenciID;
        $("input[name=islemListItem]").val(ogrid);
        modal.hide();
    });
    $(document).on('pageinit', '#profilListe', function () {
            ...
            success: function (cevap) {
                if (cevap.hata) {
                    ...
                    if (cevap.result[0][0].Stts != 0) {
                        $("#veliogrStatus").addClass("fa fa-toggle-on");
                        $("#profilDurum").text(cevap.result[0][0].Status);
                    } else {
                        $("#veliogrStatus").addClass("fa fa-toggle-off");
                        $("#profilDurum").text(cevap.result[0][0].Status);
                    }
                }
                modal.hide();
                ...
                if (cevap.hata) {
                    ...
                    if (cevap.result[0]) {
                        ...
                        for (var a = 0; a < kurumlength; a++) {
                            var elm = $('<ons-list-item modifier="chevron" onclick="$.VeliIslemler.veliKurumList(' + cevap.result[0][a].ID + ')">'
                ...
                if (cevap.hata) {
                    ...
                    if (cevap.result[0]) {
                        ...
                        for (var a = 0; a < turlength; a++) {
                            var elm = $('<ons-list-item modifier="chevron" onclick="$.VeliIslemler.turList(' + cevap.result[0][a].ID + ')">'
            ...
            success: function (cevap) {
                if (cevap.hata) {
                    ...
                    if (cevap.Detay.Gidis != "") {
                        ...
                        var elm = $('<ons-list-item modifier="chevron" onclick="$.VeliIslemler.turDetayList(0)">'
                    ...
                    if (cevap.Detay.Donus != "") {
                        $("#turDetayList").show();
                        var onsList = $("#turDetayList");
                        var elm = $('<ons-list-item modifier="chevron" onclick="$.VeliIslemler.turDetayList(1)">'
    ...
    $(document).on('pageinit', '#turDetayMap', function () {
        ...
        $.ajax({
            data: {"firma_id": firma_id, "turtip": turtip,
                "turid": turid, "veli_id": veli_id, "lang": lang,
                "tip": "turDetayMap"},
            success: function (cevap) {
                if (cevap.hata) {
                    ons.notification.alert({message: cevap.hata});
    ...
    $(document).on('pageinit', '#turGidDon', function () {
        ...
        var lang = $("input[name=lang]").val();
        $.ajax({
            data: {"turtip": turtip, "turid": turid, "turgd": turgd, "veli_id": veli_id, "firma_id": firma_id,
    ...
    $(document).on('pageinit', '#sofordetay', function () {
        ...
        var lang = $("input[name=lang]").val();
        $.ajax({
            data: {"kisiid": kisiid, "veli_id": veli_id,
    ...
    $(document).on('pageinit', '#turbildirimmesafe', function () {
        ...
        var ogrid = $("input[name=islemListItem]").val();
        $.ajax({
            data: {"veli_id": veli_id, "firma_id": firma_id, "lang": lang,
                "giddon": giddon, "ogrid": ogrid, "tip": "bilmesafayar"},
            success: function (cevap) {
                if (cevap.hata) {
                    ons.notification.alert({message: cevap.hata});
                    return false;
    ...
    $(document).on('pageinit', '#haftaliktakvim', function () {
        ...
        $.ajax({
            data: {"veli_id": veli_id, "firma_id": firma_id, "lang": lang,
                "giddon": giddon, "ogrid": ogrid, "turid": turid,
            ...
            success: function (cevap) {
                if (cevap.hata) {
                    ...
                    if (cevap.result[0]) {
                        ...
                        var elm = $('<ons-list-item modifier="chevron" onclick="$.VeliIslemler.turAktifList()">'
                                + '<ons-row><ons-col width="40px"><i class="fa fa-refresh"></i></ons-col>'
...
$.VeliIslemler = {
    ...
    bildAyarKaydet: function () {
        ...
        var giddon = $("input[name=turGidisDonus]").val();
        var ogrid = $("input[name=islemListItem]").val();
        var listv = $("#veliBildAyarList> ons-list-item").length;
        for (var v = 0; v < listv; v++) {
            var chechked = $("#veliBildAyarList> ons-list-item:eq(" + v + ")>label>input").prop("checked");
            if (chechked != false) {
                var chechkedtext = $("#veliBildAyarList> ons-list-item:eq(" + v + ") > label").text();
                var bilval = $("#veliBildAyarList> ons-list-item:eq(" + v + ") > label > input").val();
            }
        }
        $.ajax({
            data: {"veli_id": veli_id, "firma_id": firma_id, "lang": lang,
                "giddon": giddon, "ogrid": ogrid, "bilval": bilval,
    ...
    hftlikTakvimKaydet: function () {
        ...
        if (bildDegisiklik[0] == Pzt && bildDegisiklik[1] == Sli && bildDegisiklik[2] == Crs
            ...
            $.ajax({
                data: {"veli_id": veli_id, "firma_id": firma_id, "lang": lang,
                    "giddon": giddon, "ogrid": ogrid, "turid": turid, "turtip": turtip,
                    "Pzt": Pzt, "Sli": Sli,
