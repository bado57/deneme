...
$.ajaxSetup({
    type: "post",
    url: SITE_URL + "OgrenciMobilProfilAjax",
    //timeout:3000,
    dataType: "json",
    error: function (a, b) {
        if (b == "timeout")
...
ons.ready(function () {
    ...
    $(document).on('pageinit', '#editpage', function () {
        ...
        OgrenciProfil = [];
        OgrenciProfil.push(parcalama, telefon, email);
...
$.OgrenciIslemler = {
    ogrEdtEkle: function () {
        ...
        var eskiAd = OgrenciProfil[0][0].trim();
        var eskiSoyad = OgrenciProfil[0][1].trim();
        if (OgrenciProfil[0][0].trim() == ad && OgrenciProfil[0][1].trim() == soyad && OgrenciProfil[1].trim() == telefon && OgrenciProfil[2].trim() == email) {
            modal.hide();
                        ...
                        if (email != '') {
                            ...
                            $.ajax({
                                data: {"firma_id": firma_id, "eskiAd": eskiAd, "eskiSoyad": eskiSoyad,
                                    ...
                                    "email": email, "lang": lang, "ogr_id": ogr_id, "tip": "ogrProfilKaydet"},
                            ...
                            if (yenisifre == yenisifretkrar) {
                                $.ajax({
                                    data: {"firma_id": firma_id, "eskisifre": eskisifre, "yenisifre": yenisifre,
                                        "yenisifretkrar": yenisifretkrar, "ogr_id": ogr_id,
                                        "lang": lang, "kadi": kadi, "tip": "ogrSifreKaydet"},
                                    success: function (cevap) {
                                        if (cevap.hata) {
                                            modal.hide();
                                            ons.notification.alert({message: cevap.hata});
                        ...
                        case 1:
                            ...
                            $.ajax({
                                data: {"firma_id": firma_id, "ogr_id": ogr_id,
                                    "email": email, "lang": lang, "adSoyad": adSoyad, "tip": "ogrSifreUnuttum"},
                                success: function (cevap) {
                                    if (cevap.hata) {
                                        modal.hide();
                                        ons.notification.alert({message: cevap.hata});
    ...
    ogrMapSave: function () {
        ...
        if (cevap) {
            ...
            var lokasyon = $("input[name=ogrenciLokasyon]").val();
            var lang = $("input[name=lang]").val();
            if (lokasyon) {
                $.ajax({
                    data: {"firma_id": firma_id, "ulke": ulke, "il": il, "ilce": ilce, "mahalle": mahalle,
                        ...
                        "id": id, "lang": lang, "tip": "ogrHaritaKaydet"},
                    success: function (cevap) {
                        if (cevap.hata) {
                            modal.hide();
                            ons.notification.alert({message: cevap.hata});
...
function saveMap() {
    ...
    $('input[name="ogrenciLokasyon"]').val(mapEvent.latLng.lat() + "," + mapEvent.latLng.lng());
...
function Mobileinitialize(Mobileenlem, Mobileboylam) {
    ...
    var map = new google.maps.Map(document.getElementById('ogr_map'),
            mapOptions);
