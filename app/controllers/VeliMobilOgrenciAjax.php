...
class VeliMobilOgrenciAjax extends Controller {
                ...
                Switch ($tip) {
                    ...
                    case "ogrenciDetay":
                        ...
                        if (!$veliID) {
                            ...
                            $resultOgrenci = $Panel_Model->veliOgrenciDetay($id);
                            if (count($resultOgrenci) > 0) {
                                foreach ($resultOgrenci as $resultOgrencii) {
                                    $ogrenci[0][0]['ID'] = $resultOgrencii['BSOgrenciID'];
                                    $ogrenci[0][0]['Ad'] = $resultOgrencii['BSOgrenciAd'] . " " . $resultOgrencii['BSOgrenciSoyad'];
                                    $ogrenci[0][0]['Phone'] = $resultOgrencii['BSOgrenciPhone'];
                                    $ogrenci[0][0]['Email'] = $resultOgrencii['BSOgrenciEmail'];
                                    $ogrenci[0][0]['Loc'] = $resultOgrencii['BSOgrenciLocation'];
                                    $ogrenci[0][0]['Adres'] = $resultOgrencii['BSOgrenciDetayAdres'];
                                    if ($resultOgrencii['Status'] != 0) {
                                        $ogrenci[0][0]['Status'] = $languagedeger['Aktif'];
                                        $ogrenci[0][0]['Stts'] = 1;
                                    } else {
                                        $ogrenci[0][0]['Status'] = $languagedeger['Pasif'];
                                        $ogrenci[0][0]['Stts'] = 1;
                                    }
                                }
                                $sonuc["result"] = $ogrenci;
                            } else {
                    ...
                    case "kurumListe":
                        ...
                        if (!$veliID) {
                            ...
                            $resultOgrenciKurum = $Panel_Model->veliOgrenciKurum($id);
                    ...
                    case "kurumDetay":
                        ...
                        if (!$veliID) {
                            ...
                            $resultKurum = $Panel_Model->veliKurumDetay($id);
                    ...
                    case "turListe":
                        ...
                        if (!$veliID) {
                            ...
                            $ogrTurDetail = $Panel_Model->veliOgrenciTurDetail($id);
                            $ogrturId = array();
                            foreach ($ogrTurDetail as $ogrTurDetaill) {
                                $ogrturId[] = $ogrTurDetaill['BSTurID'];
                            }
                            //öğrenciişçi tur tablosu için
                            $ogrTurDetailler = $Panel_Model->veliOgrenciTurDetail2($id);
                            $ogrisciturId = array();
                    ...
                    case "turDetayListe":
                        ...
                        if (!$veliID) {
                            ...
                            $turOzellik = $Panel_Model->veliOgrenciTurDetayList($id);
                    ...
                    case "turGDListe":
                        ...
                        if (!$veliID) {
                            ...
                            $turTipOzellik = $Panel_Model->veliOgrTurTipDetayList($turid, $turgd);
                    ...
                    case "bilmesafayar":
                        ...
                        if (!$veliID) {
                            ...
                            if ($giddon != 1) {//gidiş
                                $ayar = $Panel_Model->veliOgrGidisAyar($ogrid);
                                foreach ($ayar as $ayarr) {
                                    $ayarDeger['Deger'] = $ayarr['BildirimMesafeGidis'];
                                }
                            } else {//dönüş
                                $ayar = $Panel_Model->veliOgrDonusAyar($ogrid);
                                foreach ($ayar as $ayarr) {
                    ...
                    case "bilmesafayarkaydet":
                        ...
                        if (!$veliID) {
                            ...
                            if ($bilval != $bildayarDeger) {
                                ...
                                $resultupdate = $Panel_Model->veliOgrBilMes($data, $ogrid);
                                if ($resultupdate) {
                    ...
                    case "turbildirim":
                        ...
                        if (!$veliID) {
                            ...
                            $giddon = $form->values['giddon'];
                            if ($giddon != 1) {//gidiş
                                $hftlikbild = $Panel_Model->veliHaftlikBildirimGid($veliID);
                    ...
                    case "bldrimKaydet":
                        ...
                        if (!$veliID) {
                            ...
                            $cihazSorgu = $Panel_Model->veliCihazSorgu($veliID);
                            if (count($cihazSorgu) > 0) {
                                if ($giddon != 1) {//Gidiş
                                    ...
                                    $resultupdate = $Panel_Model->veliCihazUpdate($dataUpdate, $veliID);
                                    if ($resultupdate) {
                                ...
                                if ($giddon != 1) {//Gidiş
                                    ...
                                    $resultinsert = $Panel_Model->veliCihazInsert($dataInsert);
                                    if ($resultinsert) {
                                        $sonuc["result"] = $languagedeger['BildKaydet'];
                                    } else {
                                        $sonuc["hata"] = $languagedeger['Hata'];
